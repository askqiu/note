...
# 实验：将 XSS 反射为带有尖括号、单引号、双引号、反斜杠和反引号Unicode 转义的模板文字
...
模板字符串：
1，反引号定义 
2，支持字符串插值（模板字符串允许在字符串中嵌入变量或表达式，使用 ${} 语法。这个特性让拼接变量更加简洁和直观）
3，支持多行字符串：在模板字符串中，直接输入换行符不需要额外的字符或转义。这使得编写长字符串更加方便。
有了这些知识，才能做这个靶场
